...
for i in range(0,12000):                                     #3000 - тест, в конце перейти на while(True)
    ...
    if(sp_moment_flag == True):                             #что делать, если момент особый
        ...
        segm_summ = 0                                       #debug
        #for j in reversed(range(u_prev+2,u+2)):             #u-последний отсчет, класс которого мы знаем
        for j in reversed(range(1,u-u_prev)):
            h_t_prev = K[j][h_u]                            #классификация эл-та
            H_curr.append(h_t_prev)                         #добавление в массив, которые потом склеится с H
            #print('SEGMENTATION ADDED: H = ' + str(h_t_prev))
            segm_summ += 1
        ...
        #K_new.append([0,0,0,0,0])                           #т.к новые столбцы в K добавляются с первого номера
        #for j in range(u+1,len(K)):                       #сохраняем все, что идет после столбца, соотв. новой
        #    K_new.append(K[j])                              #границе принятия решений
        #K = K_new                                           #заменяем урезанной таблицей
        k_deleted = 0                                       #debug mode on
        for j in reversed(range(1,u-u_prev)):               #удаляем с конца, чтобы не сбивалась нумерация
