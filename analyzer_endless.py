...
for i in range(0,500):                                     #3000 - тест, в конце перейти на while(True)
    ...
    if(sp_moment_flag == True):                             #что делать, если момент особый
        ...
        for j in range(1,curr_t-u_prev-1):                         #вычисляем h_s(i) для отсчетов curr_t-1,...,0
            h_t_1 = []                                          #вектор h_s(i), вычисляемый на новом шаге
            new_u_flag = True                                   #флаг, что найдена новая граница u_t
            for v in range(0,m):
                h_t_1.append(K[curr_t-j-u_prev][h_t_2[v]])
            #h_t_1_flag = h_t_1[0]
            for v in range(1,m):
                if(h_t_1[v] != h_t_1[0]):
